...
from services.database_services import write_tshirt_to_firestore, add_to_queue
from res.models.objects import TshirtWithIds, ProductQueueWithTimestamp
...
def add_patterns_to_queue(request: PatternQueueRequest, api_key: str = Depends(verify_api_key)):
    ...
    try:
        for pattern in request.queue:
            queue_item = ProductQueueWithTimestamp(
                idea=pattern.idea,
